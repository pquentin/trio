...
async def wait_child_exiting(process: "_subprocess.Process") -> None:
    ...
    """
    raise NotImplementedError from (  # pragma: nocover
...
try:
    if sys.platform == "win32":
        ...
        def create_pipe_to_child_stdin() -> Tuple[SendStream, int]:  # noqa: F811
        ...
        def create_pipe_from_child_output() -> Tuple[ReceiveStream, int]:  # noqa: F811
    ...
    elif (sys.platform != "linux" and sys.platform != "win32") or (
        not TYPE_CHECKING and sys.name == "posix"
    ):
